...
import { request, requestAdmin, requestArtist } from "./Request";
...
export async function getAllUserRoleByArtist(): Promise<UserModel[]> {
   const endpoint: string = endpointBE + `/roles`;
   const response = await requestArtist(endpoint);
...
export async function getUserbyId(idUser: any): Promise<UserModel> {
...
export async function getArtistbyId(idUser: any): Promise<UserModel> {
   const endpoint = endpointBE + `/users/${idUser}`;
   const responseUser = await request(endpoint);
